...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        f = Frame(self.master, height=1080, width=1920, bg="white", relief="ridge", bd=20)
        f.propagate(0)
        f.pack()
        self.man = PhotoImage(file="Resources\\clock.png")
        imgMan = Label(f, image=self.man, bg="white")
        imgMan.place(x=0, y=60)

        self.mainTitle = Label(f, text="Mind Scribble", fg="red" ,bg="white", font=("Poppins", 30, "bold")).place(
            x=400+400, y=200)
        self.regi = Button(f, text="Register", width=20, height=2, fg="royalblue4", bg="lavender",
                           font=("Poppins", 10, "bold "), command=self.c_reg)
        self.login = Button(f, text="Login", width=20, height=2, fg="royalblue4", bg="lavender",
                            font=("Poppins", 10, "bold "), command=self.c_login)
        self.regi.place(x=450+300, y=400)
        self.login.place(x=650+300, y=400)
...
class Register:
    ...
    def __init__(self, master):
        ...
        global f1
        f1 = Frame(self.master, height=1080, width=1920, bg="white", relief="ridge", bd=20)
        f1.propagate(0)
        f1.pack()

        self.regNow = PhotoImage(file="Resources\\register.png")
        imgRegNow = Label(f1, image=self.regNow, bg="white")
        imgRegNow.place(x=550, y=5)

        self.mainTitle = Label(f1, text="Register Yourself", bg="white",
                               font=("Poppins", 30, "bold  underline")).place(x=50, y=10)
        self.name = Label(f1, text="First Name : ", bg="white", font=("Poppins", 10))
        self.lname = Label(f1, text="Last name : ", bg="white", font=("Poppins", 10))
        self.email = Label(f1, text="Email id : ", bg="white", font=("Poppins", 10))
        self.uname = Label(f1, text="Username :", bg="white", font=("Poppins", 10))
        self.pw = Label(f1, text="Enter password : ", bg="white", font=("Poppins", 10))
        ...
        self.submit = Button(f1, text="Submit", width=20, height=2, fg="royalblue4", bg="lavender",
                             font=("Poppins", 10, "bold "), command=self.c_submit)
        self.cancel = Button(f1, text="Cancel", width=20, height=2, fg="royalblue4", bg="lavender",
                             font=("Poppins", 10, "bold "), command=self.c_cancel)

        self.checkB = Checkbutton(f1, text='Show Password', bg="white", fg="royalblue4",
                                  font=("Poppins", 10, "bold "), variable=self.var, onvalue=1,
    ...
    def check(self, l1):
        ...
        for i in range(5):
            ht = ht + 50
            if len(l1[i]) == 0:
                self.l = Label(f1, text="! You cannot leave this empty", fg='red', bg="white")
                self.l.place(x=400, y=ht)
            else:
                self.l = Label(f1, text="! You cannot leave this empty", bg="white", fg="white")
                self.l.place(x=400, y=ht)
                f = f + 1
        if l1[2].find("@") == -1 and l1[2].find(".") == -1 and len(l1[2]) != 0:
            self.l = Label(f1, text="! Please enter a valid email id", bg="white", fg="red")
            self.l.place(x=400, y=200)
            s = 1
        else:
            if (len(l1[2]) > 0):
                self.l = Label(f1, text="! Please enter a valid email id", bg="white", fg="white")
                self.l.place(x=400, y=200)
        if len(l1[4]) < 8 and len(l1[4]) != 0:
            self.l = Label(f1, text="! Password must atleast have 8 characters", bg="white", fg="red")
            self.l.place(x=400, y=300)
        else:
            if (len(l1[4]) > 0):
                self.l = Label(f1, text="! Password must atleast have 8 characters", bg="white", fg="white")
                self.l.place(x=400, y=300)
    ...
    def c_submit(self):
        conn = MySQLdb.connect(host='localhost', database='QuizDatabase', user='riddhesh', password='riddhesh')
        cursor = conn.cursor()
        name = self.tname.get()
...
class Login:
    ...
    def __init__(self, master):
        ...
        global f2
        f2 = Frame(self.master, height=1080, width=1920, bg="white", relief="ridge", bd=20)
        f2.propagate(0)
        f2.pack()

        self.l1 = Label(f2, text="Enter Username: ", bg="white", font=("Poppins", 15))
        self.e1 = Entry(f2, width=30)
        self.l2 = Label(f2, text="Enter Password: ", bg="white", font=("Poppins", 15))
        self.e2 = Entry(f2, width=30, show="*")
        self.b1 = Button(f2, text="Login", width=15, height=2, fg="royalblue4", bg="lavender",
                         font=("Poppins", 10, "bold "), command=self.clicked)
        self.b2 = Button(f2, text="Cancel", width=15, height=2, fg="royalblue4", bg="lavender",
                         font=("Poppins", 10, "bold "), command=self.cancelLogin)

        self.var = IntVar()
        self.checkB = Checkbutton(f2, text='Show Password', bg="white", fg="royalblue4",
                                  font=("Poppins", 10, "bold "), variable=self.var, onvalue=1,
                                  offvalue=0, command=self.Showpasswd)
...
class Account:
    ...
    def __init__(self, master, u):
        ...
        self.master.config(bg="#009FBF")
        f3 = Frame(mAcc, height=1080, width=1920, bg="white", relief="ridge", bd=20)
        f3.propagate(0)
        f3.pack()
        conn = MySQLdb.connect(host='localhost', database='QuizDatabase', user='riddhesh', password='riddhesh')
        cursor = conn.cursor()
        ...
        cursor.close()
        conn.close()
        self.greet = Label(f3, text="Hey " + u + ", Welcome Back!", bg="white",
                           font=("Poppins", 30, "bold ")).place(x=400, y=200)
        self.lastScore = Label(f3, text="Last Quiz Score = " + str(self.prevScore[0]), bg="white",
                               font=("Poppins", 30, "bold ")).place(x=400, y=300)
        self.takeQuiz = Button(f3, text="Take Quiz", width=20, height=2, fg="royalblue4", bg="lavender",
                               font=("Poppins", 10, "bold "), command=self.goinside)
        self.takeQuiz.place(x=500, y=400)
        self.logout = Button(f3, text="Logout", width=20, height=2, fg="royalblue4", bg="lavender",
                             font=("Poppins", 10, "bold "), command=self.logout)
...
class Quiz:
    def __init__(self, master, u):
        ...
        self.Back = Button(f, text="<- Back", width=15, height=2, fg="royalblue4", bg="snow2",
                           font=("Poppins", 10, "bold "), command=self.back).place(x=100, y=325)
        self.Next = Button(f, text="Next ->", width=15, height=2, fg="royalblue4", bg="snow2",
                           font=("Poppins", 10, "bold "), command=self.next).place(x=250, y=325)
        self.submit = Button(f, text="Submit", width=34, height=2, fg="ghost white", bg="DeepSkyBlue2",
                             font=("Poppins", 10, "bold "), command=self.Submit).place(x=100, y=400)

        imgID = random.randint(1, 4)
        filestr = "Resources\\" + str(imgID) + ".png"
        # self.quizImg = PhotoImage(file=filestr)
        # img = Label(f, image=self.quizImg, bg="white")
        # img.place(x=700, y=200)

    def create_q(self, master, qno):
        qLabel = Label(master, text=questions[qno], bg='white', font=("Poppins", 15))
    ...
    def create_options(self, master):
        ...
        while b_val < 4:
            btn = Radiobutton(master, text="", variable=self.opt_selected, value=b_val + 1, bg='white',
                              font=("Poppins", 15))
            b.append(btn)
            ht = ht + 40
            btn.place(x=30, y=ht)
